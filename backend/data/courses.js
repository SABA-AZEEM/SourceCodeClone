

export const CoursesData = [
    {
        imgURL:'/stemCourseImages/math.jpg',
        duration:'48 Months',
        title:'Mathematics',
        linkTitle:'Mathematics',
        linkTitleUrl:'/sub-category/mathematics',
        rupee:'8,900.00',
        detailUrl:'/courses-detail/mathematics',
        category:'skill-courses'
    },
    {
        imgURL:'/stemCourseImages/biology.jpg',
        duration:'44 Months',
        title:'Biology',
        linkTitle:'Biology',
        linkTitleUrl:'/sub-category/biology',
        rupee:'8,900.00',
        detailUrl:'/courses-detail/biology',
        category:'skill-courses'
    },
    {
        imgURL:'/stemCourseImages/chemistry.jpg',
        duration:'48 Months',
        title:'Chemistry',
        linkTitle:'Chemistry',
        linkTitleUrl:'/sub-category/chemistry',
        rupee:'8,900.00',
        detailUrl:'/courses-detail/chemistry',
        category:'skill-courses'
    },
    {
        imgURL:'/stemCourseImages/physics.jpg',
        duration:'48 Months',
        title:'Physics',
        linkTitle:'Physics',
        linkTitleUrl:'/sub-category/physics',
        rupee:'8,900.00',
        detailUrl:'/courses-detail/physics',
        category:'skill-courses'
    },
    {
        imgURL:'/sub-category-images/arabic-language/courses/pic1.jpg',
        level:'Beginner',
        duration:'18 Months',
        title:'Arabic for Adults',
        linkTitle:'Arabic Language',
        linkTitleUrl:'/sub-category/arabic-language',
        rupee:'6,900.00',
        detailUrl:'/courses-detail/arabic-for-adults',
        category:'skill-courses'
    },
    {
        imgURL:'/sub-category-images/arabic-language/courses/pic2.jpg',
        level:'Beginner',
        duration:'24 Months',
        title:'Arabic for Kids (age 5 to 12)',
        linkTitle:'Arabic Language',
        linkTitleUrl:'/sub-category/arabic-language',
        rupee:'6,900.00',
        detailUrl:'/courses-detail/arabic-for-kids-age-5-to-12',
        category:'skill-courses'
    },
    {
        imgURL:'/sub-category-images/arabic-language/courses/pic3.jpg',
        level:'Advanced',
        duration:'6 Months',
        title:'Arabic Advance',
        linkTitle:'Arabic Language',
        linkTitleUrl:'/sub-category/arabic-language',
        rupee:'6,900.00',
        detailUrl:'/courses-detail/arabic-advance',
        category:'skill-courses'
    },
    {
        imgURL:'/sub-category-images/arabic-language/courses/pic4.jpg',
        level:'Beginner',
        duration:'6 Months',
        title:'Arabic Tajweed',
        linkTitle:'Arabic Language',
        linkTitleUrl:'/sub-category/arabic-language',
        rupee:'6,900.00',
        detailUrl:'/courses-detail/arabic-tajweed',
        category:'skill-courses'
    },
    {
        imgURL:'/sub-category-images/arabic-language/courses/pic5.jpg',
        level:'Beginner',
        duration:'12 Months',
        title:'Arabic Grammar',
        linkTitle:'Arabic Language',
        linkTitleUrl:'/sub-category/arabic-language',
        rupee:'NA',
        detailUrl:'/courses-detail/arabic-grammar',
        category:'skill-courses'
    },
    {
        imgURL:'/featuredCoursesImages/CriticalThinking.jpg',
        duration:'12 Months',
        title:'Critical thinking - Learning how to think',
        linkTitle:'Critical Thinking',
        linkTitleUrl:'/sub-category/critical-thinking',
        rupee:'8,900.00',
        detailUrl:'/courses-detail/critical-thinking-learning-how-to-think',
        category:'skill-courses'
    },
    {
        imgURL:'/featuredCoursesImages/EmotionalIntelligence.jpg',
        duration:'12 Months',
        title:'Emotional Intelligence',
        linkTitle:'Emotional Intelligence',
        linkTitleUrl:'/sub-category/emotional-intelligence',
        rupee:'8,900.00',
        detailUrl:'/courses-detail/emotional-intelligence',
        category:'skill-courses'
    },

    {
        imgURL:'/coaching-images/personal-counseling-coaching/pic1.png',
        duration:'Hourly Session',
        title:'Personal Counseling & Coaching',
        linkTitle:'Coaching',
        linkTitleUrl:'/sub-category/coaching',
        rupee:'26,000.00',
        detailUrl:'/courses-detail/personal-counseling-coaching',
        category:'coaching-courses'
    },
    {
        imgURL:'/coaching-images/business-coaching/pic1.jpg',
        duration:'Hourly Session',
        title:'Business Coaching',
        linkTitle:'Coaching',
        linkTitleUrl:'/sub-category/coaching',
        rupee:'116,000.00',
        detailUrl:'/courses-detail/business-coaching',
        category:'coaching-courses'
    },
    {
        imgURL:'/coaching-images/teen-mentorship-program/pic1.png',
        duration:'12 Months',
        title:'Teen Mentorship Program (16-18 Yrs)',
        linkTitle:'Coaching',
        linkTitleUrl:'/sub-category/coaching',
        rupee:'15,000.00',
        detailUrl:'/courses-detail/teen-mentorship-program-16-18-yrs',
        category:'coaching-courses'
    },
]